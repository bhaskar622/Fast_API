...
async def register(data: RegisterUserCommand, db: AsyncSession = Depends(get_db)):
    user = await RegisterUserUseCase(db).execute(data)
    return user
...
async def get_me(current_user: User = Depends(get_current_user)):
    """
    Depends(get_current_user) runs the auth dependency automatically.
    If the token is invalid, 401 is raised before this code runs.
    """
    return current_user
