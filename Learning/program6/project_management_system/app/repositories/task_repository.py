...
class TaskRepository:
    ...
    async def get_paginated(
        ...
        """
        Returns tasks for current page AND total count.
        Two queries run: one for the page, one for the count.
        """
        # Count query
        count_result = await self.session.execute(select(func.count(Task.id)))
